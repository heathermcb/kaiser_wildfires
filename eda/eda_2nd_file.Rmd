...
Focusing on one file (this is the second EDA so it'll be the second file).
```{r}
head(dt[[2]])
```

This is the file containing inpatient visits for any disease.

Columns are the same as in outpatient file, but for the actual visits col.
Are dates still the same? 
...
```{r}
summary(d1$daily_byzip_ct_I_anydisease) # ranges between 0 and 10
sum(d1$daily_byzip_ct_I_anydisease == 0)/length(d1$daily_byzip_ct_I_anydisease) # ALMOST ALL are 0 (97%)
sum(is.na(d1$daily_byzip_ct_I_anydisease)) # no missing data

hist(d1$daily_byzip_ct_I_anydisease) # show 0s

d1 %>% select(daily_byzip_ct_I_anydisease) %>% filter(daily_byzip_ct_I_anydisease != 0) %>% ggplot(aes(daily_byzip_ct_I_anydisease)) + geom_histogram() # most of the rest are 1
```

How do the visits look accross the study period?
```{r}
# time series
d1 %>% group_by(county, date) %>% summarise_at(.vars = c("daily_byzip_ct_I_anydisease"), .funs = sum) %>% ggplot() + geom_point(aes(x = date, y = daily_byzip_ct_I_anydisease)) + facet_wrap(facets = c("county"))
```

We saw variation in days of the week with outpatient visits, but with these it looks like things are pretty random. Can still examine the days of the week bc I'm curious. 

```{r}
library(lubridate)
d1 %>% 
  group_by(county, date) %>% 
  summarise_at(.vars = c("daily_byzip_ct_I_anydisease"),
                                          .funs = sum) %>% 
  mutate(wkday = as.factor(wday(date))) %>% # sunday is 1
  ggplot() + 
  geom_point(aes(x = date, y = daily_byzip_ct_I_anydisease, color = wkday)) + 
...
d1 %>% 
  ...
  ggplot() + geom_point(aes(x = date, y = daily_byzip_ct_I_anydisease, color = wkday)) + 
  facet_wrap(facets = c("county"))
```

Looks random. 

Still random in Ventura? Can't tell bc of  scale. 

```{r}
d1 %>% group_by(county, date) %>% summarise_at(.vars = c("daily_byzip_ct_I_anydisease"), .funs = sum) %>% filter(county == "Ventura County") %>% ggplot() + geom_point(aes(x = date, y = daily_byzip_ct_I_anydisease)) + facet_wrap(facets = c("county"))
```
...
I re-ran all the above code to compare it to the first file; it all matches.
Going to do the same for the rest:
...
THat's funny. The total percentage of missing observations was ~14%, and the percentage of counties missing over half the observations is also 13.8% exactly. 24% are missing no observations at all. Because the patterns of missingness are the same for wf and nonwf pm, this is true for wf pm also. 
...
```{r}
wild <- d1 %>% mutate(std_visits = daily_byzip_ct_I_anydisease / tot_pop, fire_on = getty + woolsey) 
...
```{r}
wild %>% ggplot(aes(x = wf_pm25, y = daily_byzip_ct_I_anydisease)) + geom_point()
...
```{r}
wild %>% filter(getty_disaster_20km == 1 | woolsey_disaster_20km == 1) %>% ggplot(aes(x = wf_pm25, y = daily_byzip_ct_I_anydisease)) + geom_point()
wild %>% ggplot(aes(x = wf_pm25, y = std_visits)) + geom_point()
```


Everything matches the outpatient file - looking good. 
Plots at the end don't show anything surprising/don't show anything at all. 
