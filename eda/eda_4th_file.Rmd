...
This file contains inpatient circulatory-related visits.
...
```{r}
summary(d1$daily_byzip_ct_I_circulatory) # ranges between 0 and 10
sum(d1$daily_byzip_ct_I_circulatory == 0)/length(d1$daily_byzip_ct_I_circulatory) # 97% are 0
sum(is.na(d1$daily_byzip_ct_I_circulatory)) # no missing data

hist(d1$daily_byzip_ct_I_circulatory) # show 0s

d1 %>% select(daily_byzip_ct_I_circulatory) %>% filter(daily_byzip_ct_I_circulatory != 0) %>% ggplot(aes(daily_byzip_ct_I_circulatory)) + geom_histogram() # most of the rest are 1
```

How do the visits look accross the study period?
```{r}
# time series
d1 %>% group_by(county, date) %>% summarise_at(.vars = c("daily_byzip_ct_I_circulatory"), .funs = sum) %>% ggplot() + geom_point(aes(x = date, y = daily_byzip_ct_I_circulatory)) + facet_wrap(facets = c("county"))
```
...
d1 %>% 
  ...
  summarise_at(.vars = c("daily_byzip_ct_I_circulatory"),
                                          .funs = sum) %>% 
  mutate(wkday = as.factor(wday(date))) %>% # sunday is 1
  ggplot() + 
  geom_point(aes(x = date, y = daily_byzip_ct_I_circulatory, color = wkday)) + 
...
d1 %>% 
  ...
  ggplot() + geom_point(aes(x = date, y = daily_byzip_ct_I_circulatory, color = wkday)) + 
  facet_wrap(facets = c("county"))
```
...
```{r}
d1 %>% group_by(county, date) %>% summarise_at(.vars = c("daily_byzip_ct_I_circulatory"), .funs = sum) %>% filter(county == "Ventura County") %>% ggplot() + geom_point(aes(x = date, y = daily_byzip_ct_I_circulatory)) + facet_wrap(facets = c("county"))
```
...
```{r}
wild <- d1 %>% mutate(std_visits = daily_byzip_ct_I_circulatory / tot_pop, fire_on = getty + woolsey) 
...
```{r}
wild %>% ggplot(aes(x = wf_pm25, y = daily_byzip_ct_I_circulatory)) + geom_point()
...
```{r}
wild %>% filter(getty_disaster_20km == 1 | woolsey_disaster_20km == 1) %>% ggplot(aes(x = wf_pm25, y = daily_byzip_ct_I_circulatory)) + geom_point()
wild %>% ggplot(aes(x = wf_pm25, y = std_visits)) + geom_point()
```
