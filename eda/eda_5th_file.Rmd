...
Since this is the last file, I want to do a final check that the covariate parts of the datasets, so basically everything other than the visits col, are identical. 

```{r}
for (i in 1:5) {
  for (j in 1:5){
    print(identical(dt[[i]][-c(3)], dt[[j]][-c(3)]))
  }
}
...
The 5th file is inpatient cardiovascular visits. 

Dates still the same?
...
```{r}
summary(d1$daily_byzip_ct_R_circulatory) # ranges between 0 and 8
sum(d1$daily_byzip_ct_R_circulatory == 0)/length(d1$daily_byzip_ct_R_circulatory) # 95% are 0
sum(is.na(d1$daily_byzip_ct_R_circulatory)) # no missing data

hist(d1$daily_byzip_ct_R_circulatory) # show 0s

d1 %>% select(daily_byzip_ct_R_circulatory) %>% filter(daily_byzip_ct_R_circulatory != 0) %>% ggplot(aes(daily_byzip_ct_R_circulatory)) + geom_histogram() # most of the rest are 1
```

How do the visits look accross the study period?
```{r}
# time series
d1 %>% group_by(county, date) %>% summarise_at(.vars = c("daily_byzip_ct_R_circulatory"), .funs = sum) %>% ggplot() + geom_point(aes(x = date, y = daily_byzip_ct_R_circulatory)) + facet_wrap(facets = c("county"))
```

No days of the week stuff. 

```{r}
library(lubridate)
d1 %>% 
  group_by(county, date) %>% 
  summarise_at(.vars = c("daily_byzip_ct_R_circulatory"),
                                          .funs = sum) %>% 
  mutate(wkday = as.factor(wday(date))) %>% # sunday is 1
  ggplot() + 
  geom_point(aes(x = date, y = daily_byzip_ct_R_circulatory, color = wkday)) + 
...
d1 %>% 
  ...
  ggplot() + geom_point(aes(x = date, y = daily_byzip_ct_R_circulatory, color = wkday)) + 
  facet_wrap(facets = c("county"))
```

Does this pattern hold in smaller counties, obscured by the scale of the plot?

```{r}
d1 %>% group_by(county, date) %>% summarise_at(.vars = c("daily_byzip_ct_R_circulatory"), .funs = sum) %>% filter(county == "Ventura County") %>% ggplot() + geom_point(aes(x = date, y = daily_byzip_ct_R_circulatory)) + facet_wrap(facets = c("county"))
```
...
```{r}
wild <- d1 %>% mutate(std_visits = daily_byzip_ct_R_circulatory / tot_pop, fire_on = getty + woolsey) 
...
```{r}
wild %>% ggplot(aes(x = wf_pm25, y = daily_byzip_ct_R_circulatory)) + geom_point()
...
```{r}
wild %>% filter(getty_disaster_20km == 1 | woolsey_disaster_20km == 1) %>% ggplot(aes(x = wf_pm25, y = daily_byzip_ct_R_circulatory)) + geom_point()
wild %>% ggplot(aes(x = wf_pm25, y = std_visits)) + geom_point()
```
